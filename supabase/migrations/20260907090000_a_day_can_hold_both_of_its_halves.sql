/*
  A DAY CAN HOLD BOTH OF ITS HALVES.

  Darshan P V (073) had a half-day Week-off approved for 5 Sep. He then tried four times in
  eleven minutes to add the OTHER half so the date would read as one full day, gave up, and
  raised HD-2026-000007 — "Convert this into one day / Not able to mark an other half day week
  off or to change for 1". Every attempt died on:

      23P01: leave request overlaps existing request LV-2026-000042 for the same employee

  `leave_requests_no_overlap` compared date RANGES and never looked at `portion`, so an
  approved half day reserved the whole calendar date against its own complement. Two people
  splitting a date between two leave types hit the same wall.

  ── WHAT CHANGES ────────────────────────────────────────────────────────────────────────────
  1. The overlap guard exempts exactly one case: both requests are single-date half days on the
     same date, and they are OPPOSITE halves. Everything else clashes as before — a second
     first_half still clashes with the first, and any half still clashes with a full day.

     The single-date test is not decoration. `rebuild_leave_request_days` stamps
     `CASE WHEN p_from = p_to THEN p_portion ELSE 'full_day' END`, so `portion` is only
     meaningful when from = to; a multi-day row carrying 'first_half' is a full-day row wearing
     a stale label and must never be exempted on the strength of it.

  2. `compute_attendance_day` SUMS the date's approved leave rows instead of taking the first
     one it found. Without this the fix above is worse than useless: both halves would be
     approved and the day would still be priced at 0.5, because the lookup was
     `SELECT ... LIMIT 1`. Two halves now read `on_leave` at a full day's pay, which is the
     whole point of the ticket.

     The paid share is summed SEPARATELY (`v_leave_paid_value`). A paid half beside an unpaid
     half pays exactly half — the old boolean could only pay all or nothing, and with two rows
     to choose from it would have paid by whichever one sorted first.

     Where two rows exist, `leave_request_id` and `leave_type_id` on the attendance row take
     the first half's (then the earlier request). One column, two answers; the row already
     carries the summed value, and the day rows remain the full record.

  Idempotent: both are CREATE OR REPLACE. No data is rewritten here — days already computed are
  re-derived by the recompute queue when a leave request next changes them.
*/

-- ─────────────────────────────────────────────────────────────────────────────────────────────
-- 1. The overlap guard learns about halves.
-- ─────────────────────────────────────────────────────────────────────────────────────────────
...
BEGIN
  IF NEW.status IN ('pending','approved','partially_approved','cancellation_pending') THEN
    ...
    -- Only a single-date request can carry a half; see the note above.
    v_new_half := (NEW.from_date = NEW.to_date AND NEW.portion <> 'full_day');
    ...
    SELECT lr.request_number INTO v_clash
     ...
     WHERE lr.employee_id = NEW.employee_id
       ...
       AND NOT (
             -- The one permitted sharing of a date: opposite halves of the same day.
             v_new_half
         AND lr.from_date = lr.to_date
         AND lr.from_date = NEW.from_date
         AND lr.portion <> 'full_day'
         AND lr.portion <> NEW.portion
       )
     LIMIT 1;
...
-- ─────────────────────────────────────────────────────────────────────────────────────────────
-- 2. Attendance adds the halves up. Body is the live definition with the leave lookup replaced.
-- ─────────────────────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF to_regclass('public.leave_request_days') IS NOT NULL THEN
    SELECT (array_agg(lr.id ORDER BY lrd.portion, lr.created_at))[1],
           (array_agg(lt.id ORDER BY lrd.portion, lr.created_at))[1],
           bool_or(lt.is_paid),
           LEAST(SUM(lrd.day_value), 1.0),
           LEAST(COALESCE(SUM(lrd.day_value) FILTER (WHERE lt.is_paid), 0), 1.0)
      INTO v_leave_request_id, v_leave_type_id, v_leave_is_paid,
           v_leave_day_value, v_leave_paid_value
  ...
  */
  ...
  IF NOT v_is_holiday AND NOT v_is_woff AND v_first IS NOT NULL THEN
    /*
      FLOORED, unlike every other duration here. A part-minute must never be held against
      somebody: 54 seconds past the shift start is not a minute late, and the same for an
      early exit.  rounds, which is right for how long a person was
      here and wrong for what is counted against them.
    */
    v_late  := util.minutes_late(v_shift_start, v_first);
    v_early := util.minutes_late(v_last, v_shift_end);
  ...
  ELSIF v_leave_request_id IS NOT NULL AND v_leave_day_value = 1.0 THEN
    v_status := 'on_leave';  v_fraction := COALESCE(v_leave_paid_value, 0);
  ...
  ELSIF v_leave_request_id IS NOT NULL AND v_leave_day_value = 0.5 THEN
    ...
    v_fraction := COALESCE(v_leave_paid_value, 0)
