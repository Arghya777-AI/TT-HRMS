-- ============================================================================
-- The engine deducts the gaps between work periods again.
--
-- ── WHAT WAS WRONG, MEASURED ────────────────────────────────────────────────
-- On a day with more than one work period the engine credited the whole span from
-- first scan to last with nothing taken out, because the only break it recognised
-- was an explicit break_start/break_end pair -- of which this venue has recorded
-- zero -- falling back to the shift's fixed `unpaid_break_minutes`, which is 0 on
-- the General shift.
--
-- Live, on 01 Sep 2026:
--
--   employee 114   span 708m   credited 708m (11h48)   worked 505m (8h25)
--   employee PR6   span 827m   credited 827m (13h47)   worked 594m (9h54)
--   employee PR5   span 789m   credited 789m (13h09)   worked 563m (9h23)
--
-- Three to four hours a day of paid time nobody worked. Ground-shift staff were
-- close, because their 60-minute fixed deduction happens to cover one break.
--
-- ── AND WHY THE PREVIOUS ATTEMPT WAS REVERTED ───────────────────────────────
-- An earlier rule deducted EVERY interior gap and produced the opposite disaster:
-- a real day of 07:36, 07:38, 14:18, 22:12, 22:15 became five minutes worked,
-- because it removed both the 400-minute and the 474-minute gap. That was not a
-- reason to deduct nothing; it was the wrong gaps. Alternating -- p2-p3, p4-p5,
-- never p1-p2 or p3-p4 -- gives 476 minutes for that day, which is right.
--
-- ── HISTORY IS NOT REWRITTEN ────────────────────────────────────────────────
-- No recompute is run here, on instruction. Already-computed days keep the figures
-- they have; only days the engine touches from now on -- a new punch, a leave
-- approval, an admin edit, each of which already enqueues a recompute -- are
-- computed under the corrected rule.
-- ============================================================================
