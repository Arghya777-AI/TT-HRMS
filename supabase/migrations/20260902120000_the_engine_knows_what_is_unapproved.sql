-- ============================================================================
-- STEP 2: the engine works out how much of a day is still awaiting approval.
--
-- A web or phone punch taken outside the shift window carries a reason and needs an
-- administrator's decision. Until it has one the venue's rule is that the hours SHOW
-- on the day and stay OUT of the monthly total, so payroll only ever sees approved
-- time. `attendance_days.pending_approval_minutes` is that figure.
--
-- ── HOW IT IS DERIVED, AND WHY NOT PER PUNCH ────────────────────────────────
-- By computing the day a SECOND time over the punches that are not pending — same
-- span, same alternate-gap break rule — and taking the difference.
--
-- Not by attributing minutes to individual punches. On a span-minus-breaks model a
-- punch has no minutes of its own, and any per-punch share would be an invention
-- that stopped adding up the moment two pending punches sat either side of an
-- approved one.
--
-- ── ADDITIVE, AND THAT WAS VERIFIED BEFORE APPLYING ─────────────────────────
-- Diffed against `pg_get_functiondef`: 82 lines added, ZERO removed. Every existing
-- line means what it meant, `total_worked_minutes` is unchanged, and for a day with
-- no pending punch — every day recorded so far — the filtered pass returns identical
-- numbers and this comes out 0.
--
-- The INSERT was the risk: a column added to one list and not the other writes values
-- into the wrong columns silently. Counted before applying — 44 columns, 44 values,
-- with `pending_approval_minutes` <- `v_pending` sitting between
-- `payable_worked_minutes` and `is_late`.
--
-- No recompute is run. History keeps its figures, on instruction.
-- ============================================================================
...
DECLARE
  ...
  */
  v_a_first      timestamptz; v_a_last timestamptz;
  v_a_count      integer := 0;
  v_a_break      integer := 0;
  v_a_worked     integer := 0;
  v_pending      integer := 0;
BEGIN
  -- 0. Audit reason -----------------------------------------------------------
  ...
  */
  ...
  /*
    ...
    A web or phone punch taken outside the shift window carries a reason and needs an
    administrator's decision. Until it has one, the venue's rule is that the hours SHOW on the
    day and stay OUT of the monthly total, so payroll only ever sees approved time.

    Computed by running the day again over the punches that are not pending, with the same span
    and the same alternate-gap break rule, and taking the difference. Not by attributing minutes
    to individual punches: on a span-minus-breaks model a punch has no minutes of its own, and
    any per-punch share would be an invention that stopped adding up the moment two pending
    punches sat either side of an approved one.

    ADDITIVE. Every line above is untouched, `total_worked_minutes` still means what it always
    meant, and for every day with no pending punch — which is every day recorded so far — the
    filtered pass returns the same numbers and this comes out 0.
  */
  SELECT count(*), min(punched_at), max(punched_at)
    INTO v_a_count, v_a_first, v_a_last
  FROM public.attendance_punches
  WHERE employee_id = p_employee_id
    AND effective_date = p_ist_date
    AND is_voided = false
    AND NOT (requires_approval AND approved_at IS NULL);

  /*
    One scan is no span, mirroring the rule above for the unfiltered set: a lone punch has no
    departure to measure to, so it is worth nothing rather than worth the whole day.
  */
  IF v_a_count <= 1 THEN
    v_a_worked := 0;
  ELSE
    SELECT COALESCE(sum(g.gap_minutes), 0)
      INTO v_a_break
    FROM (
      SELECT util.minutes_between(p.punched_at, lead(p.punched_at) OVER w) AS gap_minutes,
             p.direction,
      ...
      WHERE p.employee_id = p_employee_id
        AND p.effective_date = p_ist_date
        AND p.is_voided = false
        AND NOT (p.requires_approval AND p.approved_at IS NULL)
      WINDOW w AS (ORDER BY p.punched_at)
    ) g
    WHERE g.gap_minutes >= pol.min_break_minutes_to_count
      AND (
    ...
    IF v_a_break = 0 AND pol.auto_deduct_break THEN
      v_a_break := sh.unpaid_break_minutes;
    END IF;

    v_a_worked := GREATEST(
      0,
      util.minutes_between(v_a_first, v_a_last) - COALESCE(v_a_break, 0));
  END IF;

  /*
    Clamped at zero and never above the day's worked minutes, which is also what
    `ck_ad__pending_within_worked` enforces. Removing punches can only ever shorten a span, so
    the subtraction cannot go negative — the clamp is against a skewed device clock, not against
    the arithmetic.
  */
  v_pending := GREATEST(0, LEAST(v_worked, v_worked - v_a_worked));
