-- =============================================================================
-- 081 · compute_attendance_day: the two punch models, as the client defines them
--
-- I had these backwards. Recorded plainly because the previous migration's comments
-- describe the wrong behaviour:
--
--   single_punch  ONE punch is enough and it makes a FULL DAY. The scan IS the
--                 attendance; duration is not the question.
--   multi_punch   BOTH an in and an out are required before attendance counts at all,
--                 and then the DURATION decides the day:
--                     < absent_below_minutes  -> absent      (venue: under 2 hours)
--                     < half_day_minutes      -> half day    (venue: 2 to 8 hours)
--                     otherwise               -> full day    (venue: 8 hours or more)
--
-- WHAT WAS WRONG. A lone scan was banded by `single_punch_treatment` in BOTH models, so
-- `single_punch` behaved exactly like `multi_punch` for the one case that defines it, and
-- one scan came out ABSENT — the precise opposite of "one punch makes a full day". And in
-- `multi_punch` the interior-gap break rule meant a day with a midday scan reported five
-- minutes worked, which is what surfaced all of this.
--
-- The thresholds are POLICY, not constants: `absent_below_minutes` and `half_day_minutes`
-- already existed on `attendance_policies` and were NULL, falling back to 120 and 240. The
-- venue's rule is 120 and 480, set explicitly below so the numbers are visible in the
-- policy row rather than inherited from a default nobody can see.
-- =============================================================================
...
-- -----------------------------------------------------------------------------
-- The venue's bands, stated on the policy instead of inherited from a fallback
-- -----------------------------------------------------------------------------

-- `app.reason` again, immediately before the write: the earlier set_config is
-- transaction-scoped and each statement may arrive in its own transaction. It also has
-- to be at least 10 characters, which the word 'migration' alone is not.
SELECT set_config('app.reason',
  'migration 081: venue attendance bands set to under 2h absent, 2-8h half day, 8h+ full day',
  true);

UPDATE public.attendance_policies
   SET absent_below_minutes = 120,   -- under 2 hours -> absent
       half_day_minutes     = 480    -- 2 to 8 hours -> half day; 8+ -> full day
 WHERE deleted_at IS NULL
   AND (absent_below_minutes IS DISTINCT FROM 120 OR half_day_minutes IS DISTINCT FROM 480);

COMMENT ON FUNCTION public.compute_attendance_day(uuid, date, text, boolean) IS
  'Computes one attendance day. single_punch: any scan makes a full day. multi_punch: needs an in AND an out, then the duration is banded by absent_below_minutes / half_day_minutes. See migration 081.';
